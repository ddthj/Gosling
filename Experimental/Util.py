...
class Vector3:
    ...
    def compare(self,value):
        if self.data[0] == value.data[0]:
            if self.data[1] == value.data[1]:
                if self.data[2]==value.data[2]:
                    return 3
                else:
                    return 2
            else:
    ...
    def div(self,value):
        return Vector3([self.data[0]/value,self.data[1]/value,self.data[2]/value])
    def mul(self,value):
        return Vector3([self.data[0]*value,self.data[1]*value,self.data[2]*value])
...
def future(ball,time):
    ...
    z = ball.location.data[2] + (ball.velocity.data[2] * time) - (325 * time * time)
    return Vector3([x,y,z])

def targetFuture(target,velocity,time):
    x = target.data[0] + (velocity.data[0] * time)
    y = target.data[1] + (velocity.data[1] * time)
...
def ballReady(agent):
    ball = agent.ball
    if abs(ball.velocity.data[2]) < 150 and timeZ(agent.ball) < 0.8:
        return True
    return False
...
def velocity1D(target_object):
    x = target_object.velocity * target_object.matrix[0]
    y = target_object.velocity * target_object.matrix[1]
    z = target_object.velocity * target_object.matrix[2]
    return Vector3([x,y,z])
